...
final class FormulaFromSmilesTest extends TestCase {
    ...
    public function testWithRightData9() {
        $this->assertEquals('C10H11NO3', FormulaHelper::formulaFromSmiles('C(=O)NC(CC2=CC=CC=C2)C(=O)O'));
    }

    public function testWithRightData10() {
        $this->assertEquals('C10H9NO2', FormulaHelper::formulaFromSmiles('C(=O)NC(CC2=CC=CC=C2)C(=O)O', LossesEnum::H2O));
    }

    public function testWithRightData11() {
        $this->assertEquals('C10H9NO3', FormulaHelper::formulaFromSmiles('C(=O)NC(CC2=CC=CC=C2)C(=O)O', LossesEnum::H2));
    }

    public function testWithRightData12() {
        $this->assertEquals('C5H9NO', FormulaHelper::formulaFromSmiles('CC(C)C(N)C(=O)O', LossesEnum::H2O));
    }
